...
import de.glawleschkoff.scannerapp.databinding.FragmentBtqlscanBinding;
...
public class BTQLScanFragment extends Fragment implements ScanManager.DataListener {

    private FragmentBtqlscanBinding binding;
    private BTQLViewModel btqlViewModel;
    private SuperViewModel superViewModel;
    private ScanManager scanManager;

    public static BTQLScanFragment newInstance() {
        return new BTQLScanFragment();
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
            ...
            if(getViewLifecycleOwner().getLifecycle().getCurrentState() == Lifecycle.State.RESUMED){
                if(btqlViewModel.getResponseUSERALBDetails().getValue().getErrorMessage() != null){
                    ...
                    Navigation.findNavController(requireView())
                            .navigate(R.id.action_btqlScanFragment_to_btqlSelectFragment);
                }
            }
        });
        ...
        binding.bt1.setOnClickListener(x -> {
            Navigation.findNavController(requireView())
                    .navigate(R.id.action_btqlScanFragment_to_menuFragment);
        });

        superViewModel.getMitarbeiter().observe(getViewLifecycleOwner(), x -> {
            if(x==null){
                Navigation.findNavController(requireView())
                        .navigate(R.id.action_btqlScanFragment_to_loginFragment);
            }
        });
    }
