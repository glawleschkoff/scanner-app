...
public class BTEDBScanFragment extends Fragment implements ScanManager.DataListener {
    ...
    public static BTEDBScanFragment newInstance() {
        return new BTEDBScanFragment();
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
            ...
            if(getViewLifecycleOwner().getLifecycle().getCurrentState() == Lifecycle.State.RESUMED){
                if(btedBViewModel.getResponseBauteil().getValue().getErrorMessage() != null){
                    new AlertDialog.Builder(getContext())
                            ...
                            .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    Navigation.findNavController(requireView()).navigate(R.id.action_BTEDBScanFragment_self);
                                }
                            })
                    ...
                    Navigation.findNavController(requireView())
                            .navigate(R.id.action_BTEDBScanFragment_to_BTEDBSelectFragment);
                }
            }
        });
        ...
        binding.bt1.setOnClickListener(x -> {
            Navigation.findNavController(requireView())
                    .navigate(R.id.action_BTEDBScanFragment_to_menuFragment);
        });

        metaViewModel.getMitarbeiter().observe(getViewLifecycleOwner(),x -> {
            if(x==null){
                Navigation.findNavController(requireView())
                        .navigate(R.id.action_BTEDBScanFragment_to_loginFragment);
            }
        });
    }
