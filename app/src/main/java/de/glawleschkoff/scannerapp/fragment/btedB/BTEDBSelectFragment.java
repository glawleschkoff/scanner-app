package de.glawleschkoff.scannerapp.fragment.btedB;
...
public class BTEDBSelectFragment extends Fragment implements ScanManager.DataListener {
    ...
    public static BTEDBSelectFragment newInstance() {
        return new BTEDBSelectFragment();
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        AndLiveData.use(getViewLifecycleOwner())
                .add(btedBViewModel.getResponseBauteil())
                .observe(getViewLifecycleOwner(),x->{
                    if(btedBViewModel.getResponseBauteil().getValue().getErrorMessage() != null){
                        new AlertDialog.Builder(getContext())
                                .setMessage("Bauteil nicht gefunden")
                                .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                                    public void onClick(DialogInterface dialog, int which) {
                                        Navigation.findNavController(requireView()).navigate(R.id.action_BTEDBSelectFragment_to_BTEDBScanFragment);
                                    }
                                })
                        ...
                        Navigation.findNavController(requireView())
                                .navigate(R.id.action_BTEDBSelectFragment_self);
                    }
                });
        ...
        btedBViewModel.getResponseBitmap().observe(getViewLifecycleOwner(), x -> {
            if(x.getResponse()!=null){
                binding.image.setImageBitmap(x.getResponse());
            }
        });

        btedBViewModel.getResponseBauteil().observe(getViewLifecycleOwner(), x -> {
            if(btedBViewModel.getResponseBauteil().getValue().getResponse()!=null){
                btedBViewModel.requestBitmap(btedBViewModel.getResponseBauteil()
                                .getValue().getResponse().getKundenAuftrag(),
                        btedBViewModel.getResponseBauteil()
                                .getValue().getResponse().getKundenPosition());
            }

            if(x.getResponse() != null){
                if(!Arrays.stream(btedBViewModel.getResponseBauteil().getValue().getResponse()
                    ...
                    if(!Arrays.stream(btedBViewModel.getResponseBauteil().getValue().getResponse()
                        ...
                        rvAdapter.setRecyclerViewItems(Arrays.asList(
                                new RVItem("ExemplarNr", btedBViewModel.getResponseBauteil().getValue().getResponse().getExemplarNr()),
                                new RVItem("Auftrag", btedBViewModel.getResponseBauteil().getValue().getResponse().getKundenAuftrag()),
                                new RVItem("Position", btedBViewModel.getResponseBauteil().getValue().getResponse().getKundenPosition())
                        ));
        ...
        metaViewModel.getMitarbeiter().observe(getViewLifecycleOwner(),x -> {
            if(x==null){
                Navigation.findNavController(requireView())
                        .navigate(R.id.action_BTEDBSelectFragment_to_loginFragment);
            }
        });
    ...
    public void onDataReceived(DecodeResult decodeResult) {
        ...
        if(decodeResult.getResult() == DecodeResult.Result.SUCCESS){
            ...
            btedBViewModel.requestBauteil(id);
        }
    }
