...
import de.glawleschkoff.scannerapp.databinding.FragmentRtalscanBinding;
...
public class RTALScanFragment extends Fragment implements ScanManager.DataListener {

    private FragmentRtalscanBinding binding;
    private RTALViewModel rtalViewModel;
    private SuperViewModel superViewModel;
    private ScanManager scanManager;

    public static RTALScanFragment newInstance() {
        return new RTALScanFragment();
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rtalViewModel.getUSERPlattenlager().observe(getViewLifecycleOwner(), x -> {
            if(getViewLifecycleOwner().getLifecycle().getCurrentState() == Lifecycle.State.RESUMED){
                if(rtalViewModel.getUSERPlattenlager().getValue().getErrorMessage() != null){
                    ...
                    new AlertDialog.Builder(getContext())
                            ...
                            .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    Navigation.findNavController(requireView()).navigate(R.id.action_rtalScanFragment_self);
                                }
                            })
                            .setIcon(android.R.drawable.ic_dialog_alert)
                            .show();
                } else if(rtalViewModel.getUSERPlattenlager().getValue().getResponse().getMenge()==0) {
                    scanManager.lockScanner();
                    new AlertDialog.Builder(getContext())
                            .setMessage("Ausgelagert von "+ rtalViewModel.getUSERPlattenlager().getValue().getResponse().getAuslagerInfo())
                            .setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    scanManager.unlockScanner();
                                    Navigation.findNavController(requireView()).navigate(R.id.action_rtalScanFragment_self);
                            ...
                            .setIcon(android.R.drawable.ic_dialog_alert)
                            .show();
                } else if(rtalViewModel.getUSERPlattenlager().getValue().getResponse().getOptimiert()==0 &&
                        rtalViewModel.getUSERPlattenlager().getValue().getResponse().getProduktion()==0 &&
                        rtalViewModel.getUSERPlattenlager().getValue().getResponse().getMenge()!=0){
                    Navigation.findNavController(requireView())
                            .navigate(R.id.action_rtalScanFragment_to_rtalSelectFragment);
                } else {
                    scanManager.lockScanner();
                    new AlertDialog.Builder(getContext())
        ...
        binding.bt1.setOnClickListener(x -> {
            Navigation.findNavController(requireView())
                    .navigate(R.id.action_rtalScanFragment_to_menuFragment);
        });

        superViewModel.getMitarbeiter().observe(getViewLifecycleOwner(), x -> {
            if(x==null){
                Navigation.findNavController(requireView())
                        .navigate(R.id.action_rtalScanFragment_to_loginFragment);
            }
        });
    }
    ...
    public void onDataReceived(DecodeResult decodeResult) {
        ...
        if(result == DecodeResult.Result.SUCCESS){
            ...
            rtalViewModel.requestUSERPlattenlager(id);
        }
    }
